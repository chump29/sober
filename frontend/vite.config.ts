      ...
      }),
      {
        ...
        transformIndexHtml: {
          ...
          handler(html: string): string {
            const t: Nullish<string> = validate<Optional<string>, TitleSchema>(env["VITE_TITLE"], TitleSchema)
            if (!t) {
            ...
            return html
              .replace("%VITE_TITLE_FIRST%", title[0] as string)
              .replace("%VITE_TITLE_LAST%", title[1] as string)
          }
        }
      },
      {
